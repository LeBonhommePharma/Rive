...
describe("pan-to-city viewport hints", () => {
  ...
  it("keeps the static pan-to-load control and the React city chips", async () => {
    ...
    assert.match(html, /class="atlas-brand"/);
    assert.match(html, /id="geo-panel"/);
    assert.match(html, /id="geo-locate"/);
    assert.match(src, /paintPanCityHint/);
    assert.match(src, /schedulePanCityHint/);
    assert.match(src, /viewportCityHint/);
    assert.match(src, /transferSearch/);
    assert.match(src, />Aller</);
    assert.match(src, /panCityTick/);
    const kitSrc = readFileSync(join(process.cwd(), "public", "Transit", "rive-kit.js"), "utf8");
    assert.match(kitSrc, /ATLAS_GAP_TEXT/);
    assert.match(kitSrc, /function viewportCityHint/);
    assert.match(kitSrc, /Charger /);
    const react = readFileSync(join(process.cwd(), "src", "components", "rive-app.tsx"), "utf8");
